...
public class GenomicRangeQuery {
	...
	public static void main(String[] args) {
		...
		int[] p1 = {2,0,0};
		int[] q1 = {4,0,6};
		ArrayUtil.traverse(solution("CAGCCTA", p1, q1));
	...
	public static int[] solution(String S, int[] P, int[] Q) {
		...
		int n = S.length();
        int[][] genoms = new int[4][n];
        List<Character> chars = Arrays.asList('A', 'C', 'G', 'T');
        ...
        for (int j=0; j<n; j++) {
           ...
           for(int i =0; i<4; i++) {
        	  if(ch == chars.get(i)) {
        		  genoms[i][j] = j-1 <0 ? 1 :  genoms[i][j-1] +1;
        	  } else {
        		  genoms[i][j] = j-1 <0 ? 0 :  genoms[i][j-1];
        	  }
           }
        }
        ...
        for (int i=0; i<querylen; i++) {
           // fromindex should be before the actual from index	
           int fromIndex = P[i]==0 ? 0 : P[i]-1;
           int toIndex = Q[i];           
           int minVal = Integer.MAX_VALUE;
           for(int row =0; row<4; row++) {
        	   int diff = genoms[row][toIndex] - genoms[row][fromIndex];   
        	   // if from and to both are 0 then no need to check diff as in that case diff is always 0
        	   if(fromIndex == toIndex || diff > 0) {
        		   minVal = Math.min(minVal, values.get(row)); 
        	   }
           }
           result[i] = minVal;
        }
